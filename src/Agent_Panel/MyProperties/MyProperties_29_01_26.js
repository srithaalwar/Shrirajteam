...
const CommissionTooltip = ({ show, commissions }) => {
  if (!show) return null;
      ...
      <div className="commission-tooltip-content">
        ...
        <div className="commission-body">
          <div className="d-flex justify-content-between align-items-center mb-2">
            <span className="fw-medium">Team 1:</span>
            <span className="fw-bold text-success">₹{commissions.team1.toLocaleString()}</span>
          </div>
          <div className="d-flex justify-content-between align-items-center mb-2">
            <span className="fw-medium">Team 2:</span>
            <span className="fw-bold text-success">₹{commissions.team2.toLocaleString()}</span>
          </div>
          <div className="d-flex justify-content-between align-items-center">
            <span className="fw-medium">Team 3:</span>
            <span className="fw-bold text-success">₹{commissions.team3.toLocaleString()}</span>
          </div>
        </div>
        <div className="commission-footer mt-2 pt-2 border-top">
          <small className="text-muted">Total: ₹{(commissions.team1 + commissions.team2 + commissions.team3).toLocaleString()}</small>
        </div>
      </div>
    </div>
  );
...
const PropertyCard = ({ property, onDeleteProperty }) => {
  ...
  // Fetch commission details (you can replace this with actual API call)
  const fetchCommissionDetails = async () => {
    try {
      // This is a mock implementation. Replace with actual API call
      // const response = await fetch(`${baseurl}/properties/${property.property_id}/commission/`);
      // const data = await response.json();
      // setCommissionDetails(data);
      
      // For now, using mock data as shown in the image
  ...
  const handleMouseEnter = () => {
    // Fetch commission details when hovering
