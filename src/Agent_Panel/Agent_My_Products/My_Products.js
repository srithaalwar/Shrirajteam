...
//           onError={(e) => {
//             e.target.src = "https://www.shutterstock.com/image-vector/missing-picture-page-website-design-600nw-1552421075.jpg";
//           }}
//         />
//         {discount > 0 && (
...
//=========================================================
// Added Price and discount Filterations
...
//             Product Commission
//           </small>
//           <small className="fw-semibold">₹{parseFloat(variant.product_commission || "0.00").toFixed(2)}</small>
//         </div>
...
//             Distribution Commission
//           </small>
//           <small className="fw-semibold">₹{parseFloat(variant.distribution_commission || "0.00").toFixed(2)}</small>
//         </div>
...
// export default AgentMyProducts;
...
//   return (
//     <div className="card h-100 shadow-sm border-0" style={{ zIndex: 999 }}>
//       {/* Product Image with Discount Badge */}
//       <div className="bg-light p-3 text-center position-relative" style={{ height: 150, cursor: "pointer" }}
//            onClick={() => navigate(`/agent-business-product-details/${product.product_id}?variant=${variant.id}`)}>
//         <img
...
/* ================= MODAL COMPONENTS ================= */
const EditProductModal = ({ product, isOpen, onClose, onSave, onDelete, baseurl }) => {
  const [formData, setFormData] = useState({
    product_name: "",
    brand: "",
    description: "",
...
const ManageMediaModal = ({ product, variant, isOpen, onClose, onUpdateMedia, onDeleteMedia, baseurl }) => {
  ...
  const handleUpdateMedia = () => {
    ...
    .then(data => {
      ...
      alert("Media updated successfully!");
      onUpdateMedia(); // Callback to refresh products
      // Reset to gallery view after successful update
      setEditMode(false);
      setSelectedMedia(null);
      setIsUploading(false);
    })
...
const ProductCard = ({ 
  ...
  return (
    <div className="card h-100 shadow-sm border-0" style={{ zIndex: 999 }}>
      {/* Product Image with Discount Badge */}
      <div className="bg-light p-3 text-center position-relative" style={{ height: 150, cursor: "pointer" }}
           onClick={() => navigate(`/agent-my-business-product-details/${product.product_id}?variant=${variant.id}`)}>
        <img
          src={getProductImage()}
          alt={getVariantName()}
          className="img-fluid mt-4"
...
const AgentMyProducts = () => {
  ...
  // E. UPDATE VARIANT WITHOUT MEDIA
  const handleUpdateVariant = (productId, variantId, variantData) => {
    console.log("Updating variant:", productId, variantId, variantData);
