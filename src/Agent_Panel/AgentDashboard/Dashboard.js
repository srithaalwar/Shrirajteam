...
const AgentDashboard = () => {
  ...
  // Prepare chart data from agent summary
  const prepareChartData = (summaryData) => {
    if (!summaryData || !summaryData.properties || !summaryData.properties.as_seller) {
      console.log("No property summary data available");
      return;
    }
    
    const { as_seller } = summaryData.properties;
    
    const labels = ["My Properties"];
    
    const available = [as_seller.available || 0];
    const sold = [as_seller.sold || 0];
    const pending = [as_seller.pending || 0];
    const verified = [as_seller.verified || 0];
    const booked = [as_seller.booked || 0];
    const rejected = [as_seller.rejected || 0];
  ...
  const prepareMetrics = () => {
    ...
    // 3. Orders as Seller Card
    if (orders && orders.as_seller) {
      metrics.push(
        { 
          label: "Orders (as Seller)", 
          value: orders.as_seller.total_orders || 0, 
          icon: faFileContract, 
          path: "/agent-orders" 
    ...
    // 4. Orders as Buyer Card
    if (orders && orders.as_buyer) {
      metrics.push(
        { 
          label: "Orders (as Buyer)", 
          value: orders.as_buyer.total_orders || 0, 
          icon: faCreditCard, 
    ...
    // 5. Properties as Seller Card
    if (properties && properties.as_seller) {
      metrics.push(
        { 
          label: "Properties (as Seller)", 
          value: properties.as_seller.total_added || 0, 
          icon: faHome, 
          path: "/agent-property-details" 
        }
      );
    ...
    // 6. Properties as Buyer Card
    if (properties && properties.as_buyer) {
      const totalPropertiesAsBuyer = (properties.as_buyer.booked || 0) + (properties.as_buyer.purchased || 0);
      metrics.push(
        { 
          label: "Properties (as Buyer)", 
          value: totalPropertiesAsBuyer, 
          icon: faBuilding, 
          path: "/agent-property-details" 
        }
      );
