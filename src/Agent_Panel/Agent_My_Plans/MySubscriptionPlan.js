...
//       const response = await axios.get(`${baseurl}/subscriptions/search/?user_id=${userId}`);
...
//                       </span>
//                     </td>
//                     <td>{formatDate(sub.subscription_start_datetime)}</td>
//                     <td>{formatDate(sub.subscription_end_datetime)}</td>
//                     <td>
...
function MySubscriptionPlan() {
  ...
  // READ URL QUERY PARAM ON MOUNT AND SET INITIAL STATUS FILTER
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const statusParam = params.get('status');
    if (statusParam) {
      setStatusFilter(statusParam); // e.g. "active", "expired", "cancelled"
    }
  }, [location.search]);
  ...
  // UPDATED: filter by both searchQuery AND statusFilter
  useEffect(() => {
    ...
    setFilteredSubscriptions(result.sort((a, b) => b.subscription_id - a.subscription_id));
    setPage(1);
    ...
    <>
      ...
      <div className="page-container">
        ...
        <div className="page-toolbar">
          ...
          {/* STATUS FILTER DROPDOWN — reflects URL param automatically via state */}
                ...
                <tr>
                  <td colSpan="9" className="no-data">
                    <div className="empty-state">
                      ...
                      <button
                        className="primary-btn"
                        style={{ marginTop: '15px', backgroundColor: '#273c75', borderColor: '#273c75', color: 'white' }}
                        onClick={() => navigate('/available-plans')}
                      >
                        Browse Available Plans
                      </button>
