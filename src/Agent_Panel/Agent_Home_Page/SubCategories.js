...
//   // Check if we're in business view (coming from banner click)
//   useEffect(() => {
//     const passedBusinessId = location.state?.businessId;
//     const passedBusinessName = location.state?.businessName;
    
//     if (passedBusinessId) {
//       console.log("Business view detected - Business ID:", passedBusinessId);
//       console.log("Business Name:", passedBusinessName);
...
//       setBusinessId(null);
//       // Check if id from URL is a business ID or category ID
//       // For category view, we'll fetch categories
//     }
//   }, [location.state]);

//   // Fetch categories only for category view (not for business view)
//   useEffect(() => {
//     if (isBusinessView) {
//       setLoading(false);
//       return;
...
//     };
//     run();
//   }, [id, buildCategoryTree, isBusinessView]);

//   // Fetch products based on view type (business or category)
//   useEffect(() => {
...
//           results = productsList.flatMap(product => {
//             if (product.variants && product.variants.length > 0) {
//               // Product has variants - create an entry for each variant
//               return product.variants.map(variant => ({
//                 product: product,
//                 variant: variant
//               }));
//             } else if (product.is_active !== false) {
//               // Product has no variants but is active - create a default variant
//               return [{
//                 product: product,
//                 variant: {
...
//           console.log("Processed products:", results.length);
//         } else {
//           // Fetch products by category
//           const categoriesToFetch = selectedCategories.length === 0 ? [id] : selectedCategories;
//           console.log("Fetching products for categories:", categoriesToFetch);
...
//             if (!res.ok) {
//               console.error(`Error fetching products for category ${catId}:`, res.status);
//               return [];
//             }
...
//     // Only fetch if we have either business view with businessId OR category view with id
//     if ((isBusinessView && businessId) || (!isBusinessView && id)) {
//       fetchProducts();
//     }
//   }, [id, selectedCategories, isBusinessView, businessId]);
...
//     switch (sortBy) {
//       case "price_asc": 
//         list.sort((a, b) => parseFloat(a.variant.selling_price) - parseFloat(b.variant.selling_price)); 
//         break;
//       case "price_desc": 
//         list.sort((a, b) => parseFloat(b.variant.selling_price) - parseFloat(a.variant.selling_price)); 
//         break;
//       case "discount_desc": 
//         list.sort((a, b) => disc(b) - disc(a)); 
//         break;
//       case "name_asc": 
//         list.sort((a, b) => a.product.product_name?.localeCompare(b.product.product_name || "")); 
//         break;
//       case "name_desc": 
//         list.sort((a, b) => b.product.product_name?.localeCompare(a.product.product_name || "")); 
//         break;
//       default:
//         // Default sorting - by creation date or id
//         list.sort((a, b) => (b.product.product_id || 0) - (a.product.product_id || 0));
//         break;
//     }
...
//   const filterProps = {
//     categoryTree, 
//     loading: loading && !isBusinessView, // Only show loading for category view
//     selectedCategories, 
//     setSelectedCategories,
//     selectedPriceRanges, 
//     setSelectedPriceRanges,
...
//   const displayTitle = isBusinessView ? businessName : categoryName;
...
//           <div className="msub-desktop-body">
//             {/* Sidebar - only show for category view */}
//             {!isBusinessView && (
//               <aside className="msub-sidebar">
//                 <div className="msub-sidebar-top">
//                   <span className="msub-sidebar-heading"><SlidersHorizontal size={15} /> Filters</span>
...
//             {/* Main */}
//             <main className={`${!isBusinessView ? "msub-main" : "msub-main-full"}`}>
//               {/* Topbar */}
//               <div className="msub-main-topbar">
//                 <div className="msub-desktop-search">
//                   <Search size={15} className="msub-search-ico" />
...
//   const [categoryName, setCategoryName] = useState("");
//   const [businessName, setBusinessName] = useState("");
//   const [viewType, setViewType] = useState("category"); // "category" or "business"
//   const [businessId, setBusinessId] = useState(null);
...
//   // Determine view type from location state and URL params
//   useEffect(() => {
//     const passedBusinessId = location.state?.businessId;
//     const passedBusinessName = location.state?.businessName;
...
//     if (passedBusinessId) {
//       // Business View - coming from card click
//       console.log("Business view detected - Business ID:", passedBusinessId);
//       console.log("Category ID from state:", passedCategoryId);
...
//       setCategoryName(passedBusinessName || "Products");
//     } else {
//       // Category View - coming from View All button
//       console.log("Category view detected - Category ID from URL:", id);
...
//           // Business View: Fetch products only for the specific business
//           console.log("Fetching products for business ID:", businessId);
//           const params = new URLSearchParams({ business: businessId });
//           const apiUrl = `${baseurl}/products/?${params}`;
//           console.log("API URL:", apiUrl);
...
//           console.log("Processed products for business:", results.length);
//         } else if (viewType === "category" && categoryId) {
//           // Category View: Fetch products for all businesses in this category
//           const categoriesToFetch = selectedCategories.length === 0 ? [categoryId] : selectedCategories;
//           console.log("Fetching products for categories:", categoriesToFetch);
...
//           const allResults = await Promise.all(promises);
//           results = allResults.flat();
//           console.log("Processed products for category:", results.length);
//         }
...
//     // Only fetch if we have either business view with businessId OR category view with categoryId
...
//   const filterProps = {
//     categoryTree, 
//     loading: loading && viewType === "category", // Only show loading for category view
//     selectedCategories, 
//     setSelectedCategories,
//     selectedPriceRanges, 
//     setSelectedPriceRanges,
...
//           {activeFilterCount > 0 && viewType === "category" && (
//             <ActiveChips selectedPriceRanges={selectedPriceRanges} setSelectedPriceRanges={setSelectedPriceRanges}
//               selectedDiscountRanges={selectedDiscountRanges} setSelectedDiscountRanges={setSelectedDiscountRanges}
//               clearAll={clearAll} />
//           )}
...
//             {/* Main */}
//             <main className={viewType === "category" ? "msub-main" : "msub-main-full"}>
//               {/* Topbar */}
//               <div className="msub-main-topbar">
//                 <div className="msub-desktop-search">
//                   <Search size={15} className="msub-search-ico" />
...
const ProductCard = ({ product, variant, navigate }) => {
  ...
  const handleAddToCart = (e) => {
    ...
    const guestCart = JSON.parse(localStorage.getItem("website_guest_cart") || "[]");
    const cartItem = {
      product_id: product.product_id, variant_id: variant.id,
      name: product.product_name, description: product.description,
      image: getImage(), mrp, price, quantity: 1, sku: variant.sku,
      attributes: variant.attributes || {}, product_attributes: product.attributes || {},
      addedAt: new Date().toISOString(), business_id: product.business, stock: variant.stock
    };
    const existingIndex = guestCart.findIndex(item => item.variant_id === variant.id);
    if (existingIndex >= 0) {
      const newQty = guestCart[existingIndex].quantity + 1;
      if (newQty > variant.stock) {
    ...
    localStorage.setItem("website_guest_cart", JSON.stringify(guestCart));
    Swal.fire({
      icon: 'success', title: 'Added to Cart!', text: `${productName} added to cart.`,
  ...
  const handleUpdateQuantity = (e, newQty) => {
    ...
    const guestCart = JSON.parse(localStorage.getItem("website_guest_cart") || "[]");
    if (newQty === 0) {
      const updated = guestCart.filter(item => item.variant_id !== variant.id);
      localStorage.setItem("website_guest_cart", JSON.stringify(updated));
      setQty(0);
      Swal.fire({ icon: 'info', title: 'Removed from Cart', showConfirmButton: false, timer: 1500 });
    } else {
      const idx = guestCart.findIndex(item => item.variant_id === variant.id);
      if (idx >= 0) {
        if (newQty > variant.stock) {
          Swal.fire({ icon: 'error', title: 'Stock Limit Exceeded', text: `Cannot add more than ${variant.stock} units`, confirmButtonColor: '#f76f2f' });
          return;
