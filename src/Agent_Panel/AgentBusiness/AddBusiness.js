...
//                     return {
//                       day: day.value,
//                       opens_at: existingHour.opens_at ? existingHour.opens_at.substring(0, 5) : null,
//                       closes_at: existingHour.closes_at ? existingHour.closes_at.substring(0, 5) : null,
...
//                                 onChange={(e) => handleWorkingHourChange(index, 'is_closed', e.target.checked)}
//                               />
...
//       const hasFiles = (logoFile && hasLogoChanged) || (bannerFile && hasBannerChanged);
...
//         }
//         if (bannerFile && hasBannerChanged) {
//           formDataToSend.append('banner', bannerFile);
//         }

//         // Don't set Content-Type header - browser will set it with boundary
//         if (isEditing) {
//           response = await axios.patch(endpoint, formDataToSend, { headers });
//         } else {
...
//           settlement_cycle_days: parseInt(formData.settlement_cycle_days),
...
//         if (isEditing) {
//           response = await axios.patch(endpoint, jsonData, { headers });
//         } else {
//           response = await axios.post(endpoint, jsonData, { headers });
//         }
//       }
...
// const AgentAddBusinessForm = ({ user, mode = 'add' }) => {
//   const { id } = useParams();
//   const [activeTab, setActiveTab] = useState('basic-details');
//   const [isEditing, setIsEditing] = useState(mode === 'edit');
//   const [isViewing, setIsViewing] = useState(mode === 'view');
...
// export default AgentAddBusinessForm;
...
const AgentAddBusinessForm = ({ user, mode = 'add' }) => {
  ...
  // Form State - Updated to include display_order
  const [formData, setFormData] = useState({
    // Basic Details
    business_name: '',
    legal_name: '',
    ...
    logo: '',
    banner: '',
    display_order: '', // Added display_order field
      ...
      try {
        ...
        if (id && mode !== 'add') {
          ...
          const formattedData = {
            ...
            logo: businessData.logo || '',
            banner: businessData.banner || '',
            display_order: businessData.display_order || '', // Added display_order
            categories: businessData.categories || [],
            support_email: businessData.support_email || '',
            support_phone: businessData.support_phone || '',
            website: businessData.website || '',
  ...
  const validateCurrentTab = () => {
    ...
    switch (activeTab) {
      case 'basic-details':
        if (!formData.business_name?.trim()) newErrors.business_name = 'Business Name is required';
        if (!formData.business_type) newErrors.business_type = 'Business Type is required';
        // display_order validation (optional, but validate if provided)
        if (formData.display_order && (parseInt(formData.display_order) < 0 || isNaN(parseInt(formData.display_order)))) {
