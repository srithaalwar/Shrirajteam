...
// import { Share2, X, Search, Copy, Check, Download } from "lucide-react";
...
//       </svg>
//     ),
//     share: async (url, title, qrBlob) => {
//       // Facebook doesn't support direct image sharing via URL
//       // So we'll share the link and user can manually attach QR
//       window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`, "_blank");
//       // Trigger QR download for manual attachment
...
//     ),
//     share: async (url, title, qrBlob) => {
//       // For WhatsApp, we need to use the WhatsApp API with image
//       // First, create a blob URL for the QR code
//       const qrUrl = URL.createObjectURL(qrBlob);
      
//       // Create message with link
//       const message = `${title}\n\n🔗 Link: ${url}\n📱 Scan the QR code below:`;
      
//       // For mobile devices, try to use Web Share API with file
//       if (navigator.canShare && navigator.canShare({ files: [new File([qrBlob], 'qrcode.png', { type: 'image/png' })] })) {
//         try {
//           await navigator.share({
//             title: title,
//             text: message,
//             url: url,
//             files: [new File([qrBlob], 'qrcode.png', { type: 'image/png' })]
//           });
//         } catch (err) {
...
//         }
//       } else {
//         // For desktop, open WhatsApp Web and they can manually attach
//         window.open(`https://wa.me/?text=${encodeURIComponent(message + ' ' + url)}`, "_blank");
//       }
...
//       // Reddit doesn't support direct image sharing
//       const link = document.createElement('a');
//       link.href = URL.createObjectURL(qrBlob);
//       link.download = 'qrcode.png';
//       link.click();
...
//     ),
//     share: async (url, title, qrBlob) => {
//       window.open(`https://mix.com/add?url=${encodeURIComponent(url)}`, "_blank");
//     },
//   },
//   {
//     name: "Mastodon",
...
//     share: async (url, title, qrBlob) => {
//       const message = `${title} ${url}`;
//       window.open(`https://mastodon.social/share?text=${encodeURIComponent(message)}`, "_blank");
//     },
//   },
//   {
//     name: "Email",
...
//     ),
//     share: async (url, title, qrBlob) => {
//       // For email, we need to create a mailto link with attachment mention
...
//     ),
//     share: async (url, title, qrBlob) => {
//       window.open(`https://translate.google.com/translate?u=${encodeURIComponent(url)}`, "_blank");
//     },
//   },
//   {
//     name: "Bluesky",
...
//     ),
//     share: async (url, title, qrBlob) => {
//       window.open(`https://getpocket.com/edit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(title)}`, "_blank");
//     },
//   },
//   {
//     name: "Teams",
...
//     ),
//     share: async (url, title, qrBlob) => {
//       window.open(`https://teams.microsoft.com/share?href=${encodeURIComponent(url)}&msgText=${encodeURIComponent(title)}`, "_blank");
//     },
//   },
//   {
...
//     ),
//     share: async (url, title, qrBlob) => {
//       window.open(`https://www.amazon.com/wishlist/add?u=${encodeURIComponent(url)}`, "_blank");
//     },
//   },
// ];
...
//   // Generate QR code blob when modal opens
//   useEffect(() => {
//     if (isOpen) {
//       setTimeout(() => {
...
//     if (!qrBlob) {
//       // If QR blob not ready, wait a bit
//       setTimeout(() => handleServiceClick(service), 500);
//       return;
//     }
    
//     await service.share(currentUrl, productTitle, qrBlob);
//   };
...
//                 <h3>Share Link + QR Code Together</h3>
//               </div>
...
//                   <Share2 size={16} />
//                   Share Link + QR Code Image (One Click)
//                 </button>
//               )}
...
//                           title={`Share on ${service.name} with QR code`}
//                         >
...
//                     Click any service to share both link + QR code together
//                   </p>
//                 </div>
...
//                     <h4 className="qr-section-title">QR Code (Will be shared)</h4>
...
//                     <p className="qr-note">
//                       ✓ For WhatsApp/Telegram on mobile: QR code will be attached automatically<br/>
//                       ✓ For other services: QR code will be downloaded for manual attachment
//                     </p>
//                   </div>
//                 </div>
//               </div>
...
//     share: async (url, title, qrBlob) => {
//       const qrUrl = URL.createObjectURL(qrBlob);
//       const message = `${title}\n\n🔗 Web Link: ${url}\n📱 Android: https://play.google.com/store/apps/details?id=com.yasla.shrirajteam\n🍎 iOS: https://apps.apple.com/in/app/shriraj/id6754551709\n\n📲 Scan QR code attached below:`;
...
//     share: async (url, title, qrBlob) => {
//       const message = `${title}\n\n🔗 Web Link: ${url}\n📱 Android: https://play.google.com/store/apps/details?id=com.yasla.shrirajteam\n🍎 iOS: https://apps.apple.com/in/app/shriraj/id6754551709`;
//       window.open(`https://mix.com/add?url=${encodeURIComponent(url)}&description=${encodeURIComponent(message)}`, "_blank");
//     },
//   },
//   {
//     name: "Mastodon",
...
//     await service.share(webUrl, productTitle, qrBlob);
//   };
...
//     try {
//       if (navigator.canShare && navigator.canShare({ files: [new File([qrBlob], 'qrcode.png', { type: 'image/png' })] })) {
//         // Create a clean message without duplicate links
//         const shareMessage = `${productTitle}\n\n🔗 Web: ${webUrl}\n📱 Android: ${playStoreUrl}\n🍎 iOS: ${appStoreUrl}\n\n📲 Please scan the QR code attached below to access the link easily.`;
...
//         });
//       } else {
//         handleCopyLink(webUrl, setCopied);
//       }
//     } catch (error) {
//       console.error('Error sharing:', error);
//       handleCopyLink(webUrl, setCopied);
...
//                 <button onClick={handleNativeShare} className="native-share-btn">
//                   <Share2 size={16} />
//                   Share All Links + QR Code Image (One Click)
//                 </button>
//               )}
...
//                     >
//                       {copiedWebUrl ? <Check size={12} /> : <Copy size={12} />}
...
//                     <div className="link-info">
//                       <div className="link-icon">
//                         <Smartphone size={16} color="#34A853" />
//                       </div>
...
//                     >
//                       {copiedPlayStore ? <Check size={12} /> : <Copy size={12} />}
...
//                     >
//                       {copiedAppStore ? <Check size={12} /> : <Copy size={12} />}
...
//===================================================================================================
// qr code image is sharing in desktop view and not working in qr  code image is not working properly in mobile while sharing 
...
//       const isInApp = /WebView|wv|Android.*wv/.test(navigator.userAgent);
      
//       if (!isInApp && navigator.canShare && navigator.canShare({ files: [new File([qrBlob], 'qrcode.png', { type: 'image/png' })] })) {
//         try {
//           await navigator.share({
//             title: title,
//             text: message,
...
//           });
//         } else {
//           await handleCopyLink(shareMessage, setCopied);
//         }
//         setTimeout(() => handleDownloadQR(), 500);
//       }
//     } catch (error) {
//       console.error('Error sharing:', error);
//       handleCopyLink(shareMessage, setCopied);
//       handleDownloadQR();
//     }
//   };
...
//   const isInMobileApp = typeof window !== 'undefined' && /WebView|wv|Android.*wv/.test(navigator.userAgent);
...
//                   ⚠️ QR code will be downloaded - please attach it manually
//                 </div>
//               )}
...
//                 <p className="qr-note">
//                   QR code will be shared automatically with supported apps
//                 </p>
//               </div>
...
//==========================================================================
// works fine in desktop view and in mobile view image is sharing like qr code 
...
//   },
// ];
// // Helper function to create a shareable QR code
// const createShareableQR = async (qrBlob) => {
...
//         const canvas = document.getElementById('share-modal-qrcode');
//         if (canvas) {
//           // Get blob for file sharing
//           canvas.toBlob((blob) => {
//             setQrBlob(blob);
//             // Also convert to data URL for embedding
//             const reader = new FileReader();
//             reader.onload = (e) => setQrDataUrl(e.target.result);
...
//   // Enhanced native share that always includes QR code
//   const handleNativeShare = async () => {
//     if (!qrBlob || !qrDataUrl) return;
...
//         });
//       } 
//       // Fallback to sharing text with embedded QR as data URL
...
//         alert('Links and QR code copied to clipboard!');
//       }
//     } catch (error) {
//       console.error('Error sharing:', error);
//       // Final fallback
