...
const AgentSearchBar = ({ placeholder = "Search products..." }) => {
  ...
  const performSearch = async (query) => {
    ...
    try {
      ...
      // REMOVED: Image extraction code - no longer processing images
      const products = filteredProducts.map(p => {
        const firstVariant = p.variants && p.variants[0];
          ...
          {isLoading ? (
            ...
            searchResults.map((result, index) => (
              <div 
                ...
                onClick={() => handleResultClick(result)}
              >
                {/* REMOVED: Icon/image section - no longer displaying images or icons */}
                <div className="agent-result-content" style={{ width: '100%' }}>
                  <div className="agent-result-title">
                    {result.title || 'Untitled'}
                  </div>
                  {/* REMOVED: Product type label - "Product" text no longer appears */}
                  {result.price && (
                    <div className="agent-result-price">
                      {formatPrice(result.price)}
                    </div>
