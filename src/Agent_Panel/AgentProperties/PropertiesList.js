...
// ============= Property Card Component =============
const PropertyCard = ({ property, onVerificationStatusUpdate, onDeleteProperty, commissionData, onBuyNowClick }) => {
  const navigate = useNavigate();
  const [isUpdating, setIsUpdating] = useState(false);
  const [verificationStatus, setVerificationStatus] = useState(property.verification_status || 'pending');
  const [isDeleting, setIsDeleting] = useState(false);
...
// ============= Property Grid Component =============
const PropertyGrid = ({ properties, viewMode, onVerificationStatusUpdate, onDeleteProperty, commissionData, onBuyNowClick }) => {
  const getGridClasses = () => {
    switch (viewMode) {
      case "grid-3":
        return "row row-cols-1 row-cols-sm-2 row-cols-md-3";
