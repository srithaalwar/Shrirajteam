...
const AddServiceProvider = () => {
  ...
  // Get user_id from localStorage - no fallback, should be present
  const userId = localStorage.getItem('user_id');
  ...
  const [formData, setFormData] = useState({
    ...
    // Status
    status: 'Pending',
    user: userId ? parseInt(userId) : null // Changed from created_by to user
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Make sure user is included and is a number
      if (formData.user) {
        payload.set('user', formData.user.toString());
      }
