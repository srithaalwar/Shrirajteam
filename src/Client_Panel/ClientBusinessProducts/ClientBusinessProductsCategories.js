...
//                 className="cbpc-search-input"
//                 value={searchTerm} 
//                 onChange={e => setSearchTerm(e.target.value)} 
//               />
//               {searchTerm && (
//                 <button className="cbpc-search-clear" onClick={() => setSearchTerm("")}>
//                   <X size={14} />
//                 </button>
//               )}
//             </div>
...
const FilterContent = ({
    ...
    return (
      <div className="cbpc-sidebar-filter">
        ...
        <SidebarSection title="Relevance" count={0}>
          <div className="cbpc-relevance-options">
            {SORT_OPTIONS.map(opt => (
              <CheckRow 
                key={opt.value} 
                label={opt.label} 
  ...
  // Mobile tabbed - Keep Relevance tab for mobile
  const tabs = [
    { key: "categories", label: "Category", count: selectedCategories.length },
    { key: "price", label: "Price", count: selectedPriceRanges.length },
    { key: "discount", label: "Discount", count: selectedDiscountRanges.length },
...
const ProductCard = ({ product, variant }) => {
  ...
  // Check if item is in cart - Added abort controller to prevent memory leaks
  useEffect(() => {
  ...
  // Function to refresh cart status after updates
...
const ClientBusinessProductsCategories = () => {
  ...
  const productSection = productsLoading ? (
    ...
    <div className="cbpc-empty"><p>No products found</p><small>Try adjusting your filters</small></div>
  ) : (
    <>
      <div className={`cbpc-grid ${isMobile ? "cbpc-grid--2col" : "cbpc-grid--desktop"}`}>
        {paginated.map(item => (
