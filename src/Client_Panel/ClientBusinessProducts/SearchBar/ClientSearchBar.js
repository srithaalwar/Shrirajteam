...
// ClientSearchBar.js - with client- prefixed class names
import React, { useState, useEffect, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
import axios from 'axios';
import { baseurl } from '../../../BaseURL/BaseURL';
...
const ClientSearchBar = ({ placeholder = "Search products..." }) => {
  ...
  const handleResultClick = (result) => {
    ...
    if (result.type === 'product') {
      ...
      if (productId && variantId) {
        ...
        navigate(`/client-product-details/${result.id}`);
      }
    }
  };
  ...
  // REMOVED: getResultIcon function
  ...
  return (
    <div className="client-search-container" ref={searchRef}>
          ...
          {isLoading ? (
            ...
            searchResults.map((result, index) => (
              <div 
                ...
                onClick={() => handleResultClick(result)}
              >
                {/* REMOVED: Icon/image section completely */}
                <div className="client-result-content" style={{ width: '100%' }}>
                  <div className="client-result-title">
                    {result.title || 'Untitled'}
                  </div>
                  {/* REMOVED: Product type label */}
                  {result.price && (
                    <div className="client-result-price">
                      {formatPrice(result.price)}
                    </div>
                  )}
                  {result.stock !== undefined && result.stock === 0 && (
                    <div className="client-result-stock client-out-of-stock">Out of Stock</div>
                  )}
                  {result.stock !== undefined && result.stock > 0 && (
                    <div className="client-result-stock client-in-stock">In Stock</div>
                  )}
                </div>
              </div>
            ))
            ...
            <div className="client-search-no-results">
              <div className="client-no-results-icon">🔍</div>
              <div>No products found for "{searchQuery}"</div>
              <div className="client-no-results-suggestion">
                Try checking your spelling or use different keywords
              </div>
            </div>
          ) : null}
