...
import './ClientSearchBarResults.css';

const ClientSearchBarResults = () => {
  ...
  const urlExcludeUserId = queryParams.get('exclude_user_id');
  ...
  // Use user_id from localStorage, fallback to URL param if needed
  const excludeUserId = getUserId() || urlExcludeUserId || null;
  ...
  const fetchSearchResults = async () => {
    ...
    try {
      // Search Products with exclude_user_id
    ...
    <div className="agent-search-results-container">
      ...
      {loading ? (
        ...
        <div className="agent-no-results">
          ...
          <button onClick={() => navigate('/client-dashboard')} className="agent-browse-button">
            Browse All Products
          </button>
        </div>
      )}
...
export default ClientSearchBarResults;