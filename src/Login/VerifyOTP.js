    ...
    try {
      ...
      if (response.data) {
        ...
        if (
          ...
          // Store user data in localStorage (same as normal login)
          if (responseData.user_id) {
            localStorage.setItem("user_id", responseData.user_id || "");
            localStorage.setItem("email", responseData.email || "");
            localStorage.setItem("username", responseData.username || "");
            localStorage.setItem("phone_number", responseData.phone_number || "");
            localStorage.setItem("referral_id", responseData.referral_id || "");
            localStorage.setItem("referred_by", responseData.referred_by || "");
            localStorage.setItem("user_name", responseData.first_name || responseData.user_name || "");
          }
          ...
          if (userRoles.length > 0) {
            ...
            if (userRoles.length > 1) {
              ...
              // No roles assigned, check referral ID
            ...
            // If no roles in response, check localStorage or referral ID
            const storedRoles = localStorage.getItem("userRoles");
            if (storedRoles) {
              const parsedRoles = JSON.parse(storedRoles);
              if (parsedRoles.length === 1) {
                navigateToDashboard(parsedRoles[0]);
              } else {
                navigate("/", { replace: true });
              }
            } else {
              // Try to determine role from referral ID
              const referralId = responseData.referral_id || localStorage.getItem("referral_id") || "";
              if (referralId.toUpperCase().startsWith("SRP")) {
                navigate("/agent-dashboard", { replace: true });
              } else if (referralId.toUpperCase().startsWith("SRT")) {
                navigate("/Client-dashboard", { replace: true });
      ...
      if (error.response) {
        ...
        if (errorData.message?.includes("success") || errorData.message?.includes("verified")) {
          // It's actually a success - store user data
          if (errorData.user_id) {
            localStorage.setItem("user_id", errorData.user_id || "");
            localStorage.setItem("email", errorData.email || "");
            localStorage.setItem("phone_number", errorData.phone_number || "");
            localStorage.setItem("referral_id", errorData.referral_id || "");
          }
          ...
          const userRoles = errorData.roles || [];
          if (userRoles.length === 1) {
            navigateToDashboard(userRoles[0]);
          } else {
            navigate("/", { replace: true });
