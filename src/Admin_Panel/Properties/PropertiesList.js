...
//               <span className="fw-medium">Team {commission.level}:&nbsp;₹{commission.amount.toLocaleString('en-IN', { maximumFractionDigits: 2 })}&nbsp;({commission.percentage}%)</span>
      ...
      <div className="card-body d-flex flex-column flex-grow-1">
        ...
        <div className="mb-3">
          <div className="d-flex align-items-center justify-content-between mb-2">
            <span className="small fw-semibold">Verification Status:</span>
            <span
              className={`badge ${getVerificationBadgeColor(verificationStatus)} small`}
            >
              {getVerificationDisplayText(verificationStatus)}
            </span>
          </div>
          ...
          {loadingRoles ? (
            ...
            filteredRoles.map((role) => (
              <div
                ...
                onKeyPress={(e) =>
                  e.key === "Enter" && toggleRole(role.role_name)
                }
                tabIndex={0}
                role="checkbox"
                aria-checked={selectedRoles.includes(role.role_name)}
              >
                <div className="d-flex align-items-center gap-2">
                  <input
                    type="checkbox"
                  ...
                  />
                  <span
                    className={`small ${selectedRoles.includes(role.role_name) ? "fw-semibold text-dark" : "text-muted"}`}
                  >
                    {role.role_name}
                  </span>
                </div>
...
const ProductHeader = ({
  ...
  return (
    ...
    //   </div>
    // </div>
    <div className="properties-mobile-head d-flex align-items-center justify-content-between mb-4">
      <div className="properties-title-row d-flex align-items-center gap-3">
        <h4 className="fw-bold mb-0">Properties</h4>
        <p className="mb-0 text-muted">
          Showing{" "}
      ...
      <div className="properties-action-row d-flex align-items-center gap-3">
        <div
          className="input-group properties-search-mobile"
           style={{ width: "300px" }}
        >
          <span className="input-group-text bg-transparent border-end-0">
            <Search className="h-4 w-4 text-muted" />
        ...
        <div
          className="btn-group properties-view-mobile"
          role="group"
        ...
        <button
          onClick={onOpenMobileFilters}
          className="btn btn-primary properties-filter-mobile d-md-none d-flex align-items-center gap-2"
        >
          <Filter size={16} />
          Filters
          {activeFilterCount > 0 && (
                ...
                <div className="row g-3">
                  ...
                  <div className="col-md-6">
                    ...
                    <div className="mb-3">
                      <div className="d-flex align-items-center gap-2 mb-2">
                        <span className="small fw-semibold">Verification:</span>
                        <span
                          className={`badge ${getVerificationBadgeColor(property.verification_status)} small`}
                        >
                  ...
                  <div className="col-md-3 d-flex flex-column gap-2">
                    ...
                    <button
                      onClick={() =>
                        navigate(
                          `/admin-properties-details/${property.property_id}`,
                        )
                      }
                      className="btn fw-semibold py-2"
                      style={{
...
const Adminproperties = () => {
  ...
  // Use filteredProperties (with price filtering) but note that sorting is handled by API
    ...
    return (
      <nav aria-label="Properties pagination" className="mt-5">
        ...
        <div className="text-center text-muted small mt-2">
          Page {currentPage} of {totalPages} • {displayedProperties.length}{" "}
    ...
    <div className="min-vh-100 d-flex flex-column">
      ...
      <main className="flex-grow-1 bg-light">
        <div className="container py-4">
          ...
          <ProductHeader
            totalProducts={totalCount}
            showingProducts={displayedProperties.length}
            viewMode={viewMode}
            onViewModeChange={setViewMode}
            searchTerm={searchTerm}
            setSearchTerm={setSearchTerm}
