...
// const AdminBusinessEnquiriesList = () => {
...
// export default AdminBusinessEnquiriesList;
...
const AdminBusinessEnquiriesList = () => {
  ...
  const fetchEnquiries = async () => {
    try {
      ...
      // Process the enquiries data - EXPAND each product into its own row
      const expandedEnquiries = [];
      
      (Array.isArray(enquiriesData) ? enquiriesData : []).forEach(enquiry => {
        if (enquiry.products && enquiry.products.length > 0) {
          // Create a separate row for each product in the enquiry
          enquiry.products.forEach((product, index) => {
            expandedEnquiries.push({
              ...enquiry,
              product_name: product.name || '',
              quantity: product.qty || 0,
              brand: product.brand || '',
              product_index: index + 1, // Track which product number this is
              total_products_in_enquiry: enquiry.products.length
            });
          });
        } else {
          // If no products, still show the enquiry with empty product info
          expandedEnquiries.push({
            ...enquiry,
            product_name: 'N/A',
            quantity: 0,
            brand: '',
            product_index: 1,
            total_products_in_enquiry: 0
          });
        }
      });
      
      setEnquiries(expandedEnquiries);
    } catch (error) {
      console.error('Error fetching enquiries:', error);
      Swal.fire({
        icon: 'error',
      ...
      <div className="container-fluid enquiries-container">
        ...
        <div className="row mb-4">
          <div className="col-12">
            ...
            <h1 className="enquiries-title">Product Enquiries</h1>
            {enquiries.length > 0 && (
              <p className="text-muted">Total products enquired: {enquiries.length}</p>
            )}
          </div>
        </div>
                ...
                <table className="table table-bordered">
                  ...
                  <tbody>
                    {enquiries.map((enquiry, idx) => (
                      <tr key={`${enquiry.enquiry_id}-${idx}`}>
                        <td>
                          {enquiry.enquiry_id}
                          {enquiry.total_products_in_enquiry > 1 && (
                            <span className="badge bg-info ms-2" style={{fontSize: '0.7rem'}}>
                              Product {enquiry.product_index}/{enquiry.total_products_in_enquiry}
                            </span>
                          )}
                        </td>
                        <td>{enquiry.product_name || 'N/A'}</td>
                        <td>{enquiry.quantity || 0}</td>
                        <td>{enquiry.brand || 'N/A'}</td>
                        <td>{enquiry.contact_name}</td>
                        <td>{enquiry.contact_phone}</td>
                        <td>{enquiry.contact_email}</td>
                        <td>{enquiry.message || 'No message'}</td>
