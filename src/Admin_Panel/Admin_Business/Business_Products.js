...
//     if (businessId) { params.append("business", businessId); fetch(`${baseurl}/business/${businessId}/`).then(r => r.json()).then(d => { if (d.business_name) setBusinessName(d.business_name); }).catch(() => {}); }
//     else params.append("user_id", getUserId());
...
//   useEffect(() => { setCurrentPage(1); }, [searchTerm, sortBy, selectedPriceRanges, selectedDiscountRanges]);

//   const filteredSorted = useMemo(() => {
//     let list = products.filter(item => {
//       const s = searchTerm.toLowerCase();
//       if (s && !item.product.product_name.toLowerCase().includes(s) && !(item.product.brand || "").toLowerCase().includes(s) && !(item.variant.sku || "").toLowerCase().includes(s)) return false;
//       if (selectedPriceRanges.length > 0) {
//         const price = parseFloat(item.variant.selling_price);
//         if (!selectedPriceRanges.some(r => r === "10000+" ? price >= 10000 : (([a, b]) => price >= a && price <= b)(r.split("-").map(Number)))) return false;
//       }
//       if (selectedDiscountRanges.length > 0) {
//         const m = parseFloat(item.variant.mrp), sp = parseFloat(item.variant.selling_price);
//         const d = m > 0 && sp < m ? Math.round(((m - sp) / m) * 100) : 0;
//         if (!selectedDiscountRanges.some(r => r === "60+" ? d >= 60 : (([a, b]) => d >= a && d <= b)(r.split("-").map(Number)))) return false;
//       }
//       return true;
//     });
//     switch (sortBy) {
...
//       case "discount_desc": { const d = i => { const m = parseFloat(i.variant.mrp), s = parseFloat(i.variant.selling_price); return m > 0 ? ((m - s) / m) * 100 : 0; }; list.sort((a, b) => d(b) - d(a)); break; }
...
//     }
//     return list;
//   }, [products, searchTerm, sortBy, selectedPriceRanges, selectedDiscountRanges]);
...
//     <div className="mp-empty"><p>No products found</p><small>{searchTerm || activeFilterCount > 0 ? "Try adjusting search or filters" : businessName ? `No products for ${businessName}` : "No products added yet"}</small></div>
//   ) : (
//     <>
...
//               <input type="text" placeholder="Search products..." className="mp-search-input" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
...
//           <div className="mp-count">{filteredSorted.length} product{filteredSorted.length !== 1 ? "s" : ""}</div>
//           {productSection}
//           <div style={{ height: 24 }} />
