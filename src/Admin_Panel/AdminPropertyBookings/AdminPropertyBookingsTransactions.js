...
//     const searchPlaceholder = "Search by  property...";
...
//     // Fetch transactions from API - Only property related, success status, and Booking-Amount
//     const fetchTransactions = async () => {
//         setLoading(true);
//         setError(null);
//         try {
...
//             // Filter only Booking-Amount payment type
//             params.append('payment_type', 'Booking-Amount');
...
//                 paid_amount: item.paid_amount,
//                 remaining_amount: item.remaining_amount || item.paid_amount,
//                 property_value: item.property_value || 'N/A',
//                 company_commission: item.company_commission || '0.00',
//                 payment_mode: mapPaymentMode(item.payment_mode, item.payment_method),
//                 payment_method: item.payment_method || 'N/A',
//                 role: userRolesMap.get(item.user_id) || 'User',
//                 username: item.username || `user_${item.user_id}`,
...
//         const headers = [
//             "S.No", "Transaction ID", "Transaction Date", "Property Name",
//             "Booking Amount", "Property Value", "Remaining Amount", "Payment Mode",
//             "Payment Method", "Role", "User ID", "Status", "Invoice No."
//         ];
...
//                     `"${item.property_name}"`,
//                     `"${parseFloat(item.paid_amount).toLocaleString('en-IN')}"`,
//                     `"${parseFloat(item.property_value).toLocaleString('en-IN')}"`,
//                     `"${parseFloat(item.remaining_amount).toLocaleString('en-IN')}"`,
//                     `"${item.payment_mode}"`,
//                     `"${item.payment_method}"`,
//                     `"${item.role}"`,
...
//         link.setAttribute("href", url);
//         link.setAttribute("download", `booking_amount_transactions_${timestamp}.csv`);
//         link.style.visibility = "hidden";
//         document.body.appendChild(link);
//         link.click();
//         document.body.removeChild(link);
...
//             icon: "success",
//             title: "Export Successful",
//             text: `Exported ${filteredTransactions.length} booking amount transactions to CSV file.`,
//             timer: 2000,
//             showConfirmButton: false
//         });
//     };
...
//     const searchPlaceholder = "Search by property name or invoice number...";
...
//                 {/* Header */}
//                 <div className="staff-header">
//                     <h2>Property Booking Amount Transactions</h2>
//                     <div style={{ fontSize: '14px', color: '#666', marginTop: '8px' }}>
//                         Total Booking Amount Transactions: {totalItems}
//                     </div>
//                     <div style={{ display: 'flex', gap: '10px', marginTop: '8px' }}>
//                         <button 
//                             className="btn btn-primary btn-sm"
...
//                                                 <span className="visually-hidden">Loading...</span>
//                                             </div>
//                                             <p className="mt-2">Loading booking amount transactions...</p>
//                                         </div>
//                                     </td>
//                                 </tr>
//                             ) : filteredTransactions.length > 0 ? (
...
//                                         </td>
//                                         <td className="payment-type-cell">
//                                             <span className="booking-amount-badge">Booking Amount</span>
//                                         </td>
//                                         <td className="amount-cell">
//                                             <strong className="booking-amount">₹{parseFloat(transaction.paid_amount).toLocaleString('en-IN')}</strong>
//                                         </td>
//                                         <td className="amount-cell">
//                                             <strong>₹{parseFloat(transaction.property_value).toLocaleString('en-IN')}</strong>
//                                         </td>
//                                         <td className="amount-cell">
//                                             <strong style={{ color: '#f59e0b' }}>₹{parseFloat(transaction.remaining_amount).toLocaleString('en-IN')}</strong>
//                                         </td>
...
//                                             </div>
//                                         ) : (
//                                             "No booking amount transactions found"
//                                         )}
//                                     </td>
//                                 </tr>
//                             )}
...
function AdminPropertyBookings() {
    ...
    const [selectedTransaction, setSelectedTransaction] = useState(null);
    const [paymentLoading, setPaymentLoading] = useState(false);
    const [paymentFiles, setPaymentFiles] = useState([]); // Changed to array for multiple files
    const [paymentFormData, setPaymentFormData] = useState({
        property_name: '',
        property_id: '',
        user_id: '',
    ...
    const handleMakeFullPayment = (transaction) => {
        ...
        setPaymentFiles([]); // Reset files array
        setShowPaymentModal(true);
    };
    ...
    // Handle multiple payment files change
    const handlePaymentFilesChange = (e) => {
                                ...
                                <form>
                                    ...
                                    <div className="mb-3">
                                        ...
                                        {paymentFiles.length > 0 && (
                                            <div className="mt-3">
                                                <label className="form-label fw-bold">Selected Files ({paymentFiles.length}):</label>
