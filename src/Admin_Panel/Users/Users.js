...
//               <span>💡 You can also search by: user ID, status (active/inactive), date (YYYY-MM-DD), role name</span>
//             </div>
//           </div>
//         )}
...
//   // Store referral ID to user name mapping
//   const [referralMap, setReferralMap] = useState({});

//   // Fetch all users to create referral mapping
//   const fetchAllUsersForReferralMap = async () => {
//     try {
//       const response = await axios.get(`${baseurl}/users/search/?page_size=1000`);
//       let allUsers = [];
...
//       // Create mapping: referral_id -> full name
//       const map = {};
//       allUsers.forEach(user => {
//         if (user.referral_id) {
//           const fullName = `${user.first_name || ''} ${user.last_name || ''}`.trim();
//           map[user.referral_id] = fullName || user.email || user.phone_number || user.referral_id;
//         }
//       });
...
//       return map;
//     } catch (err) {
//       console.error("Error fetching users for referral mapping:", err);
//       return {};
//     }
//   };
...
//       // Get referral mapping first if not already available
//       let currentReferralMap = referralMap;
//       if (Object.keys(currentReferralMap).length === 0) {
//         currentReferralMap = await fetchAllUsersForReferralMap();
//       }
      
//       // Transform data
//       const transformed = data.map((user) => {
//         // Get referrer name from referral map
//         const referredByReferralId = user.referred_by;
//         let referredByName = "N/A";
        
//         if (referredByReferralId && currentReferralMap[referredByReferralId]) {
//           referredByName = currentReferralMap[referredByReferralId];
//         } else if (referredByReferralId) {
//           // If we have the referral ID but no name found, show the referral ID
//           referredByName = referredByReferralId;
//         }
...
//           role: mapRole(user.roles?.[0]?.role_name || ""),
//           referralId: user.referral_id || "N/A",
//           referredBy: referredByName, // Store the referrer's name
//           referredByReferralId: referredByReferralId, // Store original referral ID for reference
//           kycStatus: user.kyc_status || "pending",
//           fullData: user,
//           created_at: user.created_at || "",
//           displayDate: formatDateForDisplay(user.created_at || ""),
...
const getUserDisplayName = (user) => {
  ...
  // Priority 2: Use full_name if available and not empty
  else if (fullName && fullName.trim() !== "") {
    return fullName;
  }
  // Priority 3: Use first_name only
  else if (firstName && firstName.trim() !== "") {
    return firstName;
  }
  // Priority 4: Use last_name only
  else if (lastName && lastName.trim() !== "") {
    return lastName;
  }
  // Priority 5: No name available
  else {
    return "N/A";
  }
};
    ...
    try {
      ...
      allUsers.forEach(user => {
        if (user.referral_id) {
          ...
          map[user.referral_id] = displayName || user.email || user.phone_number || user.referral_id;
        }
      });
    ...
    try {
      ...
      const transformed = data.map((user) => {
        ...
        return {
          ...
          name: displayName, // This will be used in the table
          email: user.email || "",
          phone: user.phone_number || "",
          status: user.status || "inactive",
          role: mapRole(user.roles?.[0]?.role_name || ""),
          ...
          <table className="staff-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ backgroundColor: '#f8f9fa' }}>
                <th style={{ padding: '12px', borderBottom: '1px solid #dee2e6', textAlign: 'left' }}>S.No.</th>
                <th style={{ padding: '12px', borderBottom: '1px solid #dee2e6', textAlign: 'left' }}>User ID</th>
                <th style={{ padding: '12px', borderBottom: '1px solid #dee2e6', textAlign: 'left' }}>FULL NAME</th>
                <th style={{ padding: '12px', borderBottom: '1px solid #dee2e6', textAlign: 'left' }}>EMAIL</th>
                <th style={{ padding: '12px', borderBottom: '1px solid #dee2e6', textAlign: 'left' }}>MOBILE</th>
                <th style={{ padding: '12px', borderBottom: '1px solid #dee2e6', textAlign: 'left' }}>ROLE</th>
                <th style={{ padding: '12px', borderBottom: '1px solid #dee2e6', textAlign: 'left' }}>REFERRAL ID</th>
                ...
                filteredStaff.map((user, index) => (
                  <tr key={user.id} style={{ borderBottom: '1px solid #eee' }}>
                    ...
                    <td style={{ padding: '12px', fontWeight: '500' }}>
                      {user.name !== "N/A" ? (
                        <span title={`First Name: ${user.first_name || 'N/A'}, Last Name: ${user.last_name || 'N/A'}, Full Name: ${user.full_name || 'N/A'}`}>
                          {user.name}
                        </span>
                      ) : (
                        <span style={{ color: '#999', fontStyle: 'italic' }}>No name provided</span>
                      )}
                    </td>
